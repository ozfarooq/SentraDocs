...
The information is conveniently presented in a tabular format within the application as depicted in the above screenshot. It is also possible to export the available grid into a  downloadable **CSV** file.  Here are the list of user properties to view in this page.
...
**Role:** This column indicate the user role that can be one of **`Admin`**, **`Member`** or **`Viewer`**. The Admin user will be able to view this in the dropdown which enable the Admins to change the user role as per the requirement. On this screen, the Admin can enable or disable the user  and even change the user role assignment as per the requirements.
